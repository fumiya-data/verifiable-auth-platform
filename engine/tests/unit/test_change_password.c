...
static void test_change_password_requires_authentication(void)
{
    ...
    TEST_CHECK(auth_change_password(&state, "old", "new") ==
           AUTH_CHANGE_PASSWORD_RESULT_UNAUTHORIZED);
}

static void test_change_password_rejects_null_arguments(void)
{
    auth_state_t state;

    auth_state_init(&state);

    TEST_CHECK(auth_change_password(nullptr, "old", "new") ==
               AUTH_CHANGE_PASSWORD_RESULT_SYSTEM_ERROR);
    TEST_CHECK(auth_change_password(&state, nullptr, "new") ==
               AUTH_CHANGE_PASSWORD_RESULT_SYSTEM_ERROR);
    TEST_CHECK(auth_change_password(&state, "old", nullptr) ==
               AUTH_CHANGE_PASSWORD_RESULT_SYSTEM_ERROR);
}
...
static void test_change_password_wrong_password(void)
{
    ...
    TEST_CHECK(auth_change_password(&state, "wrong", "new-secret") ==
           AUTH_CHANGE_PASSWORD_RESULT_WRONG_PASSWORD);
    TEST_CHECK(strcmp(state.users[0].salt, original_salt) == 0);
    TEST_CHECK(strcmp(state.users[0].password_hash, original_hash) == 0);
}
...
static void test_change_password_success(void)
{
    ...
    TEST_CHECK(auth_change_password(&state, "hunter2", "new-secret") ==
           AUTH_CHANGE_PASSWORD_RESULT_SUCCESS);
    TEST_CHECK(state.authenticated);
    TEST_CHECK(strcmp(state.authenticated_login_id, "alice") == 0);
    TEST_CHECK(strcmp(state.users[0].salt, original_salt) != 0);
    TEST_CHECK(strcmp(state.users[0].password_hash, original_hash) != 0);
...
static void test_change_password_system_error_when_session_user_is_missing(void)
{
    ...
    TEST_CHECK(auth_change_password(&state, "old-password", "new-password") ==
           AUTH_CHANGE_PASSWORD_RESULT_SYSTEM_ERROR);
}
...
int main(void)
{
    ...
    test_change_password_rejects_null_arguments();
