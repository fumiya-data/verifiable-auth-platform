...
目的は、単に似た名前のファイルを並べることではありません。次を説得力をもって
レビュー可能な形で示すことです。

- Lean モデルが意図された挙動を明示していること
- C エンジンが運用境界において同じ挙動を実装していること
- 既知の不一致が、黙った乖離ではなく意図的な refinement であること
- 残る妥協点が可視化され、防御可能であること

散文レベルのシステム契約は [`仕様書.md`](./仕様書.md)、
...
この追加説明こそが重要です。形式仕様が説得力ある証拠になるのは、読者が
「実行可能な実装がどう refinement しているか」と
「どこで実装がモデルより具体化される必要があるか」を見て取れるときです。
...
最重要の実装上の妥協点を、ここで明示的に列挙します。これらは bug ではありません。
...
- PBKDF2 実装そのものを証明対象にすると、検証対象の中心である state-machine property から焦点が外れる
...
より強い主張は、実装を仕様の faithful refinement としてレビューできることです。
