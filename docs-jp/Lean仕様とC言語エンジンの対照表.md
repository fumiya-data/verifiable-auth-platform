...
| Lean 仕様 | C 実装 | 対応関係の根拠 / 実装上の妥協点 |
| --- | --- | --- |
| `spec/VerifiableAuth/Types.lean` における名目的な `LoginId` / `Password` wrapper | `engine/include/auth/types.h`、`engine/include/auth/register.h`、`engine/include/auth/login.h`、`engine/include/auth/change_password.h` における `char login_id[...]` と `const char *password` | 両層とも login identifier と password を文字列値として扱いますが、Lean では両者を型として分離します。C 層は低レベル実装として境界付き storage が必要なため、固定最大長 (`AUTH_LOGIN_ID_MAX_LENGTH`, `AUTH_PASSWORD_MAX_LENGTH`) を追加しています。 |
...
| `spec/VerifiableAuth/Types.lean` の `User` 構造体と nested `Credential` | `engine/include/auth/types.h` の `auth_user_t` | Lean は `salt` と `passwordHash` を credential material としてグループ化します。C 表現では同じ情報を `auth_user_t` の flat field として保持します。その他は `loginId/login_id`, `failedAttempts/failed_attempts`, `lockState/lock_state` に対応します。 |
...
| `spec/VerifiableAuth/StateModel.lean` の `User.clearFailedAttempts` | `engine/src/core/login.c` の success branch | Lean helper は counter を 0 にし、同時に symbolic helper result を `active` に戻すため、単体でも invariant-preserving です。C の login success branch は locked-user guard の後で counter を 0 にするため、明示的な lock-state write がなくても結果 state は active のままです。 |
...
| `spec/VerifiableAuth/StateModel.lean` の `User.CredentialSaltBound` と `AuthState.CredentialSaltBound` | `engine/src/storage/file_user_store.c` における persisted-record validation と、`auth_register(...)` / `auth_change_password(...)` の間接的 enforcement | C 側に同名の直接 predicate はありません。しかし engine は対応する construction discipline を operational に保っています。salt / hash は専用 adapter 経由で生成・再生成され、永続化レコードは runtime state へ戻る前に検証されます。Lean は salt-owner binding を宣言的に述べ、平文非保持や provenance の強い主張をこの狭い invariant に含めないようにしています。 |
...
| `spec/VerifiableAuth/GlobalInvariants.lean` の `ConstructionInvariants`, `I02_UniqueLoginIds`, `I05_LockStateTracksThreshold`, `I11_SessionIsOptionalLoginId`, `I13_WellFormed`, `CoreInvariants` | `engine/include/auth/types.h`, `engine/include/auth/state.h`, core transition, storage loader, `engine/tests` | これは単一関数対応ではなく cross-cutting correspondence です。`CoreInvariants` は現在、証明済みの credential salt binding と `AuthState.WellFormed` を組み合わせます。`AuthState.WellFormed` は uniqueness、threshold tracking、session consistency を含みます。 |
| register, login, change-password, logout に対する construction / well-formedness / `CoreInvariants` preservation theorem | `engine/tests/unit/test_register.c`, `engine/tests/unit/test_login.c`, `engine/tests/unit/test_change_password.c`, `engine/tests/unit/test_logout.c`, さらに `engine/tests/integration` の storage parse test | Lean は full core invariant preservation を抽象的に証明します。C エンジンは focused test と、合法的 state mutation を対応コード経路に制限することによって同じ挙動を経験的に示します。 |
...
| `spec/VerifiableAuth/RefinementNotes.lean` の `loginAuditEventType`, `loginAuditEventTypeForState`, `loginTransitionAuditEventType` | `engine/src/cli/command_runner.c` の `cli_login_audit_event_type(...)` と login 前の `was_locked` 計算 | 直接的な挙動整合です。Lean は transition-level helper で pre-state から lock flag を導出し、engine も login 前の lookup によって「すでに locked な user」と「今回の試行で新たに lockout になった user」を区別します。 |
