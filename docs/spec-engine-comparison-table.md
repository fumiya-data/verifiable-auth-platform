...
| Lean specification | C implementation | Alignment rationale / implementation compromise |
| --- | --- | --- |
| Nominal `LoginId` and `Password` wrappers in `spec/VerifiableAuth/Types.lean` | `char login_id[...]` and function parameters `const char *password` in `engine/include/auth/types.h`, `engine/include/auth/register.h`, `engine/include/auth/login.h`, and `engine/include/auth/change_password.h` | Both layers model login identifiers and passwords as textual values, but Lean now keeps them type-separated. The C layer adds fixed maximum lengths (`AUTH_LOGIN_ID_MAX_LENGTH`, `AUTH_PASSWORD_MAX_LENGTH`) because a low-level implementation needs bounded storage. |
...
| `structure User` plus nested `Credential` in `spec/VerifiableAuth/Types.lean` | `auth_user_t` in `engine/include/auth/types.h` | Lean groups `salt` and `passwordHash` as credential material while the C representation stores the same fields directly in `auth_user_t`. The other direct correspondences are `loginId/login_id`, `failedAttempts/failed_attempts`, and `lockState/lock_state`. |
| `structure AuthState` with `users : List User` and `authenticated : Option LoginId` in `spec/VerifiableAuth/Types.lean` | `auth_state_t` in `engine/include/auth/state.h` with `users[AUTH_MAX_USERS]`, `user_count`, `authenticated`, and `authenticated_login_id` | The semantic shape is the same: a user collection plus a single optional session. The main compromise is that Lean uses an unbounded `List`, while C uses a fixed-capacity array plus `user_count`. This is a pragmatic systems-level choice that keeps the engine simple and allocation-free. The resulting capacity limit is an implementation bound; outside that supported range, the engine refuses the operation with `system_error` rather than mutating out of bounds. |
...
| `User.clearFailedAttempts` in `spec/VerifiableAuth/StateModel.lean` | success branch of `auth_login(...)` in `engine/src/core/login.c` | Lean's helper clears the counter and forces the symbolic helper result to `active`, making it total-invariant-preserving. The C login success branch clears the counter after the locked-user guard has excluded locked users, so the resulting state remains active without an explicit lock-state write. |
...
| `User.CredentialSaltBound` and `AuthState.CredentialSaltBound` in `spec/VerifiableAuth/StateModel.lean` | indirect enforcement through `auth_register(...)`, `auth_change_password(...)`, and persisted-record validation in `engine/src/storage/file_user_store.c` | There is no direct C predicate with this name, but the engine maintains the corresponding construction discipline operationally: salts/hashes are generated or regenerated through the dedicated adapters, and persisted records are validated before reentry into runtime state. Lean states the salt-owner binding declaratively and keeps stronger plaintext/provenance claims out of this narrow invariant. |
...
| `ConstructionInvariants`, `I02_UniqueLoginIds`, `I05_LockStateTracksThreshold`, `I11_SessionIsOptionalLoginId`, `I13_WellFormed`, and `CoreInvariants` in `spec/VerifiableAuth/GlobalInvariants.lean` | `engine/include/auth/types.h`, `engine/include/auth/state.h`, core transitions, storage loaders, and engine tests under `engine/tests` | This is a cross-cutting correspondence rather than a single function-to-function mapping. `CoreInvariants` now combines the proven credential salt binding with `AuthState.WellFormed`, which already contains uniqueness, threshold tracking, and session consistency. |
| Construction, well-formedness, and `CoreInvariants` preservation theorems for register, login, change-password, and logout in `spec/VerifiableAuth/GlobalInvariants.lean` | engine tests in `engine/tests/unit/test_register.c`, `engine/tests/unit/test_login.c`, `engine/tests/unit/test_change_password.c`, `engine/tests/unit/test_logout.c`, plus storage parse tests in `engine/tests/integration` | Lean now proves full core-invariant preservation abstractly. The C engine demonstrates the same behavior empirically through focused tests and by constraining legal state mutations to the corresponding code paths. |
...
| Lean specification | C implementation | Alignment rationale / implementation compromise |
| --- | --- | --- |
| `register` definition in `spec/VerifiableAuth/RegisterSpec.lean` | `auth_register(...)` in `engine/src/core/register.c` | The same two modeled branches exist inside the valid fixed-capacity domain: duplicate leaves state unchanged, otherwise a fresh user is appended. The C implementation adds a runtime `system_error` guard for invalid arguments, exhausted capacity, and credential-generation failure. |
...
| Lean specification | C implementation | Alignment rationale / implementation compromise |
| --- | --- | --- |
| `RegisterResult`, `LoginResult`, `ChangePasswordResult`, and `LogoutResult` in `spec/VerifiableAuth/Types.lean` | `auth_register_result_t`, `auth_login_result_t`, `auth_change_password_result_t`, and `auth_logout_result_t` in `engine/include/auth/result.h` | This is the core result-space alignment. `RegisterResult.systemError` is a refinement-level implementation-bound outcome, not a third pure register branch in the valid model. |
...
| `loginAuditEventType`, `loginAuditEventTypeForState`, and `loginTransitionAuditEventType` in `spec/VerifiableAuth/RefinementNotes.lean` | `cli_login_audit_event_type(...)` and the pre-login `was_locked` computation in `engine/src/cli/command_runner.c` | Direct behavioral alignment. Lean now derives the lock flag from the pre-state for the transition-level helper, matching the engine's pre-login lookup that distinguishes already-locked users from newly triggered lockouts. |
...
- Outside the supported range, the C engine must return `system_error` rather than relying on assertions or writing past the fixed array.
